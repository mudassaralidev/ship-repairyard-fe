...
export const getAvailableEmployees = async ({ foreman_id, department_id }) => {
  const { data } = await axios.get(
    `v1/users/employees/available?foreman_id=${foreman_id}&department_id=${department_id}`,
  );
...
export const assignClientAPI = async (userId, clientUserId) => {
  const { data } = await axios.put(
    `v1/users/${userId}/assign-client/${clientUserId}`,
  );
